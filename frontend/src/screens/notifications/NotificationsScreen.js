...
export default function NotificationsScreen({ navigation }) {
  ...
  const handleNotificationPress = (notification) => {
    ...
    if (notification.data?.propertyId) {
      navigation.navigate('PropertyDetails', { propertyId: notification.data.propertyId });
    } else if (notification.data?.userId) {
      navigation.navigate('Chat', {
        recipientId: notification.data.userId,
        recipientName: notification.data.userName,
      });
    }
  };
  ...
  const renderNotification = ({ item }) => (
    ...
    >
      <View style={[styles.iconContainer, { backgroundColor: item.read ? COLORS.light : COLORS.primary + '20' }]}>
        <Ionicons
