...
export default function AddPropertyScreen({ navigation }) {
  ...
  useEffect(() => {
    if (user && user.role !== 'seller') {
      Alert.alert(
        'Accès restreint',
        'Seuls les vendeurs peuvent ajouter des propriétés. Voulez-vous changer votre rôle en vendeur?',
        [
  ...
  const handleSubmit = async () => {
    ...
    try {
      const propertyData = {
        ...formData,
        images: images,
        image: images[0] // First image as main for backward compatibility
      };

      console.log('Submitting property with images:', propertyData);
      console.log('API base URL:', api.api.defaults.baseURL);
      console.log('Auth token set:', !!api.api.defaults.headers.common['Authorization']);
      ...
      Alert.alert(
        'Erreur', 
        error.response?.data?.message || 'Impossible d\'ajouter la propriété. Veuillez réessayer.'
      );
    }
  };
